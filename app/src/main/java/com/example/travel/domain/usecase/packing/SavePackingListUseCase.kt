...
class SavePackingListUseCase @Inject constructor(
    private val packingRepository: PackingRepository
) {
    suspend operator fun invoke(item: PackingItem): Resource<Boolean> {
        return packingRepository.savePackingItem(item)
    }
}
